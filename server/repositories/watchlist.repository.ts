...
export type WatchlistRow = Awaited<ReturnType<typeof WatchlistRepository.list>>["data"][number];
...
export const WatchlistRepository = {
  async list(options: { workspaceId: string; page?: number; pageSize?: number }) {
    ...
    const [data, total] = await Promise.all([
      ...
      prisma.watchlist.count({ where: { workspaceId: options.workspaceId } }),
    ]);

    return { data, total, page, pageSize, totalPages: Math.ceil(total / pageSize) };
  },

  async create(input: { workspaceId: string; name: string; description?: string | null }) {
  ...
  async update(workspaceId: string, id: string, patch: { name?: string | null; description?: string | null }) {
    const existing = await prisma.watchlist.findFirst({ where: { id, workspaceId }, select: { id: true } });
  ...
  async addDomain(workspaceId: string, watchlistId: string, params: { domain: string; label?: string | null }) {
    ...
    const wl = await prisma.watchlist.findFirst({ where: { id: watchlistId, workspaceId }, select: { id: true } });
