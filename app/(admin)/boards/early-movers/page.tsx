...
import { getCachedEarlyMoversBoard } from "@/lib/perf/cached-server-data";
...
export default async function EarlyMoversBoardPage({ searchParams }: Props) {
  await trackBoardViewServer("early-movers", "/boards/early-movers");
  ...
  let rows: Awaited<ReturnType<typeof getCachedEarlyMoversBoard>> = [];
  let error: string | null = null;

  try {
    rows = await getCachedEarlyMoversBoard(take, minScore);
  } catch (e) {
  ...
  return (
    <div>
      <PageHeader
        title="Early Movers"
        description="Fresh product clusters that are starting to produce signals fast — catch them before saturation kicks in."
      />

      <div className="mb-5 rounded-lg border border-border bg-card/80 p-4 shadow-sm">
        <p className="text-sm text-muted leading-relaxed max-w-3xl">
          Optimized for discovery and speed: new clusters with very recent activity, accelerating linkage signals, and
          low-to-mid saturation. Run{" "}
          <span className="font-mono text-xs text-foreground">refresh_product_clusters</span> to refresh scores.
        </p>
        <form className="mt-4 flex flex-wrap items-end gap-3 text-sm" method="get" action="/boards/early-movers">
      ...
      {error ? (
        ...
        <EmptyState
          ...
          description="Lower the minimum score or run product cluster refresh so early-mover scores populate."
        />
      ) : (
          ...
          <table className="w-full text-sm min-w-[980px]">
            <thead>
              <tr className="bg-surface-2 border-b border-border text-left">
                ...
                <th className="px-3 py-2.5 text-[11px] font-semibold text-muted uppercase text-right">Early score</th>
                <th className="px-3 py-2.5 text-[11px] font-semibold text-muted uppercase text-right">7d Δ</th>
                <th className="px-3 py-2.5 text-[11px] font-semibold text-muted uppercase text-right">Age</th>
                <th className="px-3 py-2.5 text-[11px] font-semibold text-muted uppercase text-right">Stores</th>
                <th className="px-3 py-2.5 text-[11px] font-semibold text-muted uppercase">Freshness</th>
            ...
            <tbody className="divide-y divide-border">
              {rows.map((r) => {
                ...
                const early = Math.round(r.earlyMoverScore * 10) / 10;
                const ageDays = daysSince(r.firstSeenAt);
                const freshnessDays = daysSince(r.lastSeenAt);
                const freshnessLabel = freshnessDays < 1 ? "today" : `${Math.round(freshnessDays)}d`;
                ...
                return (
                  <tr key={r.clusterId} className="hover:bg-surface-2/70 transition-colors group">
                    ...
                    <td className="px-3 py-2.5 text-right align-top">
                      <Badge label={early.toFixed(1)} variant={trendBadgeVariant(early)} />
                    </td>
                    <td className="px-3 py-2.5 text-right align-top">
                    ...
                    <td className="px-3 py-2.5 text-xs text-muted align-top whitespace-nowrap">
                      <div className="flex flex-col">
                        <span className="text-foreground tabular-nums">{freshnessLabel}</span>
                        <span className="text-muted-2">{timeAgo(r.lastSeenAt)}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-2.5 align-top">
                      <div className="flex flex-wrap gap-1 max-w-[260px]">
                        ...
                        {r.readyToScaleScore > 0 && (
                          <Badge label={`rts ${Math.round(r.readyToScaleScore)}`} variant="default" />
                        )}
                        {r.winningScore > 0 && <Badge label={`win ${r.winningScore}`} variant="default" />}
                        {r.linkedCreativeClusterCount === 0 &&
                          r.linkedRawRecordCount === 0 &&
                          r.readyToScaleScore === 0 &&
                          r.winningScore === 0 && <span className="text-muted-2 text-xs">—</span>}
                      </div>
                    </td>
                  </tr>
                );
