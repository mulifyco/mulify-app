...
import { getCachedReadyToScaleBoard } from "@/lib/perf/cached-server-data";
...
export default async function ReadyToScaleBoardPage({ searchParams }: Props) {
  await trackBoardViewServer("ready-to-scale", "/boards/ready-to-scale");
  ...
  let rows: Awaited<ReturnType<typeof getCachedReadyToScaleBoard>> = [];
  let error: string | null = null;

  try {
    rows = await getCachedReadyToScaleBoard(take, minScore);
  } catch (e) {
  ...
  return (
    <div>
      <PageHeader
        ...
        description="High-potential product clusters where winning demand meets scalable creative signals — without extreme saturation."
      />

      <div className="mb-5 rounded-lg border border-border bg-card/80 p-4 shadow-sm">
        <p className="text-sm text-muted leading-relaxed max-w-3xl">
          Prioritizes clusters with strong cross-store momentum and fresh activity, creative scale via linked ad
          clusters on the same shops, and controlled saturation. Run{" "}
          <span className="font-mono text-xs text-foreground">refresh_product_clusters</span> to refresh scores.
        </p>
        <form className="mt-4 flex flex-wrap items-end gap-3 text-sm" method="get" action="/boards/ready-to-scale">
      ...
      {error ? (
        ...
        <EmptyState
          ...
          description="Lower the minimum score or run product cluster refresh so ready-to-scale scores populate."
        />
      ) : (
          ...
          <table className="w-full text-sm min-w-[920px]">
            ...
            <tbody className="divide-y divide-border">
              {rows.map((r) => {
                ...
                const ready = Math.round(r.readyToScaleScore * 10) / 10;
                return (
                  <tr key={r.clusterId} className="hover:bg-surface-2/70 transition-colors group">
                    ...
                    <td className="px-3 py-2.5 text-right align-top">
                      <Badge label={ready.toFixed(1)} variant={trendBadgeVariant(ready)} />
                    </td>
                    <td className="px-3 py-2.5 text-right align-top">
                    ...
                    <td className="px-3 py-2.5 align-top">
                      <div className="flex flex-wrap gap-1 max-w-[220px]">
                        ...
                        {r.landingPageCount > 0 && (
                          <Badge label={`${r.landingPageCount} LP`} variant="default" />
                        )}
                        {r.linkedCreativeClusterCount === 0 &&
                          r.linkedRawRecordCount === 0 &&
                          r.collectionCount === 0 &&
                          r.landingPageCount === 0 && <span className="text-muted-2 text-xs">—</span>}
                      </div>
                    </td>
                  </tr>
                );
